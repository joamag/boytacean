...
impl Crc32C {
    ...
    fn update_sw(&mut self, bytes: &[u8]) {
        ...
        for &byte in bytes {
            ...
            value = CRC32C_TABLE[index as usize] ^ (value >> 8);
        }
        self.value = value;
    }
    ...
    unsafe fn update_hw_x86_64(&mut self, bytes: &[u8]) {
        ...
        while i + 8 <= bytes.len() {
            ...
            value = _mm_crc32_u64(value as u64, chunk) as u32;
            i += 8;
        }
        while i < bytes.len() {
            value = _mm_crc32_u8(value, bytes[i]);
            i += 1;
        }
        self.value = value;
    }
    ...
    unsafe fn update_hw_aarch64(&mut self, bytes: &[u8]) {
        ...
        while i + 8 <= bytes.len() {
            ...
            value = __crc32cd(value, chunk);
            i += 8;
        }
        while i < bytes.len() {
            value = __crc32cb(value, bytes[i]);
            i += 1;
        }
        self.value = value;
    }
...
mod tests {
    ...
    fn test_crc32c_empty() {
        let data: [u8; 0] = [];
        assert_eq!(crc32c(&data), 0x00000000);
    }

    #[test]
    fn test_crc32c_single_byte() {
        let data: [u8; 1] = [0xab];
        assert_eq!(crc32c(&data), 0x69bf4dcc);
    }

    #[test]
    fn test_crc32c_multiple_bytes() {
        let data: [u8; 5] = [0x12, 0x34, 0x56, 0x78, 0x9a];
        assert_eq!(crc32c(&data), 0x426094af);
    }
