...
export class GameBoyCore extends EmulatorLogic {
     ...
     * instead of a path relative to this module, so that it's found
     * both when the package is installed under `node_modules` and
     * when it's linked from the repository, falling back to the
     * sibling `lib` directory used by the web front-end.
     *
    ...
    protected static defaultWasmPath(): string | null {
        ...
        // resolves the binary through a bare specifier, an idiom that
        // bundlers statically analyse and rewrite into the emitted
        // asset, making it the strategy that works in the browser
        //
        // notice that the specifier must be kept as an inline literal,
        // as bundlers only match this exact shape, and that whenever it
        // is not rewritten the expression silently yields a path next
        // to this very module instead of raising, so that result is
        // detected and discarded by comparison with the sibling one
        try {
            const path = new URL("boytacean/boytacean_bg.wasm", import.meta.url)
                .href;
            const sibling =
                new URL("./", import.meta.url).href +
                "boytacean/boytacean_bg.wasm";
            if (path !== sibling) return path;
        } catch (err) {
            // ignores the error and falls back to the relative
