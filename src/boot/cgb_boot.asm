...
; Load Tilemap
    ld hl, $98C2
; Load the number of rows in the tilemap in D
    ld d, 3
    ld a, 8

.tilemapLoop
; Load the number of columns in the tilemap in C (16)
    ld c, $10
...
; Switch to back first VRAM Bank
    ...
    ldh [$4F], a
    pop af
; Write the concrete value of the tile index into the tilemap,
; increments the HL after writing
    ldi [hl], a
; Adds the index number by 3, we need this because the tiles are sorted in a different way
; column order instead of row order
    add d

; Decrement the column counter (C) and repeats row loop if not finished
    dec c
    jr nz, .tilemapRowLoop

; Subtracts the number of columns from the current tile index
; this is 3 x 16 = 48 - 1
    sub 47
    push de
    ld de, $10
...
; Setup the tile loading for the Nintendo logo
    ld a, $38 ; start tile index for Nintendo logo
    ld l, $a7 ; offset within the tile map to update index for Nintendo logo
    ld bc, $0107 ; 1 row of 7 columns (1x7 tiles)
    jr .tilemapRowLoop
.endTilemap
ENDC
...
; End RLE decoding, the rest uses HL as the destination
.LogoEnd
    ld h, d
    ld l, $80
