...
class PyBoyV1(GameBoy):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if cgb is None:
            cgb = _detect_cgb_from_rom(gamerom_file)
...
class PyBoyV2(GameBoy):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if cgb is None:
            cgb = _detect_cgb_from_rom(gamerom)
...
def _detect_cgb_from_rom(rom: Union[str, IO[bytes], None]) -> bool:
    if rom is None:
        return False
    try:
        if isinstance(rom, str):
            ...
            data = rom.read()
            rom.seek(pos)
            mode = infer_mode_data(data)
    except (OSError, ValueError, RuntimeError):
        return False
    return GameBoyMode(mode) == GameBoyMode.CGB
